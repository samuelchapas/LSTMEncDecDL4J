...
public class CharIterator implements DataSetIterator {
	...
	private static Map<Character,Integer> charToIdxMap;
	 ...
	 */
	public CharIterator(char[] textFilePath, Charset textFileEncoding, int miniBatchSize, int exampleLength,
     ...
     * @throws IOException If text file cannot  be loaded
     */
    public CharIterator(char[] text, Charset textFileEncoding, int miniBatchSize, int exampleLength,
		...
		for( int i=0; i<validCharacters.length; i++ ) {
			...
			//System.out.println(validCharacters[i] + " " + i);
		}
		...
		int maxSize = text.length;	//add lines.size() to account for newline characters at end of each line
		//for( String s : lines ) {
		//	maxSize += s.length();
		...
		//for( String s : lines ){
			...
			//	System.out.println(aThisLine + "\n");
			//}
			//if(newLineValid) characters[currIdx++] = '\n';
		//}

		if( text.length > 0){
			System.out.println("Inside fileCharacters");
			fileCharacters = text;
		} 
		for(int i = 0; i < fileCharacters.length; i++) {
			char c = fileCharacters[i];
			System.out.print(c);
		}
		//else {
		...
		//+ maxSize + " total characters (" + nRemoved + " removed) " + "exampleLength="+exampleLength);
	...
	public static int convertCharacterToIndex( char c ){
	...
	public DataSet next(int num) {
        ...
        for( int i=0; i<currMinibatchSize; i++ ){
            ...
            //System.out.println("fileCharacters " + fileCharacters);
            
            //for(int t = 0; t < fileCharacters.length; t++) {
    		//	char c = fileCharacters[t];
    		//	System.out.print(c);
    		//}
            
           // System.out.println("charToIdxMap " + charToIdxMap + " startIdx " + startIdx);
            
            int currCharIdx = charToIdxMap.get(fileCharacters[i]);	//Current input
            //int c=0;
            //for( int j=startIdx+1; j<endIdx; j++, c++ ){
            int nextCharIdx = charToIdxMap.get(fileCharacters[(i + 1) % (fileCharacters.length)]);		//Next character to predict
